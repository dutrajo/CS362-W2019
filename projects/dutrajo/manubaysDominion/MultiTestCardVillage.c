...
/*
	int int validateVillageCardEffect(
				int playerIndex,
...
)
{
	...
	sprintf(str, "\tRunning cardEffect For Village....");
	...
	sprintf(str, "\tChecking player's hand count increased by 1 cards...");
	customPrintf(str, outputFile);

	sprintf(strPass, "Passed.\n");
	sprintf(strFail, "Failed!!!\n");

	passesCheck = assertTrue(tempG.handCount[playerIndex], G->handCount[playerIndex], strPass, strFail, outputFile);
	overallTestPass &= passesCheck;
	...
	//output status to screen and file
	sprintf(str, "\tChecking player's deck count reduced by 1 cards...");
	customPrintf(str, outputFile);

	sprintf(strPass, "Passed.\n");
	sprintf(strFail, "Failed!!!\n");

	passesCheck = assertTrue(tempG.deckCount[playerIndex], G->deckCount[playerIndex] - 1, strPass, strFail, outputFile);
	overallTestPass &= passesCheck;
	...
	for (i = 0; i < treasure_map + 1; i++)
	{
		...
		if (i == handCardPlayed)
		{
			...
			if (deckCountChanges != (handCountChanges - 1))
			{
	...
	passesCheck = assertTrue(tempG.numActions == G->numActions + 2, 1, strPass, strFail, outputFile);
	overallTestPass &= passesCheck;
...
*/
void multiTestCardEffectVillage()
{
	...
	FILE *outputFile = fopen("MultiTestCardVillage.out", "w");
	...
	for (i = 0; i < numberOfRandomTests; i++)
	{
		...
		backupG.hand[playerIndex][backupG.handCount[playerIndex]++] = village;
		...
		res = validateVillageCardEffect(playerIndex, backupG.handCount[playerIndex] - 1, village, choice1, choice2, choice3, &bonusCoins, &backupG, outputFile, 0);
